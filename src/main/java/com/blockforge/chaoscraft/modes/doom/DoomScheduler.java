...
/**
 * Attack scheduler for Doom Mode.
 * Unlike other mode schedulers that only select BLOCK_DISPLAY attacks,
 * this scheduler uses weighted random selection across all 3 attack types
 * (BLOCK_DISPLAY, ENVIRONMENTAL, MODEL_ENGINE) with configurable weights.
 */
public class DoomScheduler {
    ...
    public DoomScheduler(ChaosCraftPlugin plugin, AttackRegistry registry, DoomConfig config) {
        this.plugin = plugin;
    ...
    private void attemptSpawn() {
        ...
        // Fallback: if selected type has no attacks, try other types
        if (attack == null) {
        ...
        plugin.debug("[DoomScheduler] Spawned " + attack.getId() + " (" + attack.getType()
                + ") near " + target.getName() + " (active: " + activeAttacks.size() + ")");
    }

    /**
     * Select an attack type using weighted random based on config weights.
     */
    ...
    public void forceSpawn(AbstractAttack attack, Player player) {
        ...
        plugin.debug("[DoomScheduler] Force-spawned " + instance.getId() + " on " + player.getName());
    }
