...
public class CalamityConfig {
    ...
    public void load() {
        ...
        if (!config.contains("on-player-ready-commands")) { config.set("on-player-ready-commands", new java.util.ArrayList<>()); needsSave = true; }
        if (!config.contains("on-reset-commands")) { config.set("on-reset-commands", new java.util.ArrayList<>()); needsSave = true; }
        ...
        if (!config.contains("rewards.survived.items")) { config.set("rewards.survived.items", new java.util.ArrayList<>()); needsSave = true; }
        if (!config.contains("rewards.survived.badges")) { config.set("rewards.survived.badges", new java.util.ArrayList<>()); needsSave = true; }
        if (!config.contains("rewards.survived.commands")) { config.set("rewards.survived.commands", new java.util.ArrayList<>()); needsSave = true; }
        if (!config.contains("rewards.died.money")) { config.set("rewards.died.money", 0); needsSave = true; }
        if (!config.contains("rewards.died.items")) { config.set("rewards.died.items", new java.util.ArrayList<>()); needsSave = true; }
        if (!config.contains("rewards.died.badges")) { config.set("rewards.died.badges", new java.util.ArrayList<>()); needsSave = true; }
        if (!config.contains("rewards.died.commands")) { config.set("rewards.died.commands", new java.util.ArrayList<>()); needsSave = true; }
        ...
        if (!config.contains("restrictions.blocked-commands")) { config.set("restrictions.blocked-commands", new java.util.ArrayList<>()); needsSave = true; }
